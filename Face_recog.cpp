...
int faceHeight = 90;	//	"		"		"		"		"		"		"		"
int nPersons                  = 0; // the number of people in the training set. int nTrainFaces               = 0; // the number of training images
...
void printUsage()
{
	printf("Usage: OnlineFaceRec [<command>] \n"
		"  Valid commands are: \n"
		"    train <train_file> \n"
		"    test <test_file> \n"
...
void storeEigenfaceImages()
{
	...
	if (nEigens > 0) {
		...
		IplImage *bigImg = cvCreateImage(size, IPL_DEPTH_8U, 1);			for (int i=0; i<nEigens; i++) {
...
IplImage* cropImage(const IplImage *img, const CvRect region)
{
	...
	// create a new (color or greyscale) IPL Image and copy contents of img into it.
